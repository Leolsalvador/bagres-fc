      ...
      <div className="min-h-full bg-background flex flex-col">
        ...
        <div className="mx-4 mt-4 bg-card rounded-3xl p-6 flex flex-col items-center">
          ...
          <p className="text-text-muted text-xs mt-1">
            {votos[rerating.id] === 'skip' ? 'Marcado como Desconheço' : `Avaliação atual: ${votos[rerating.id]} ⭐`}
          </p>
        </div>
          ...
          <div className="w-full space-y-2">
            {Object.entries(votos).map(([id, r]) => {
              ...
              return (
                <button
                  key={id}
                  onClick={() => { setRerating(p); setReratingStars(isSkip ? 0 : r) }}
                  className="w-full bg-card rounded-2xl px-4 py-3 flex items-center justify-between active:scale-95 transition-transform"
                >
                  <p className={cn('text-sm font-semibold', isSkip ? 'text-text-muted' : 'text-text-main')}>{p?.nome}</p>
                  {isSkip
                    ? <span className="text-text-muted text-xs italic">Desconheço</span>
  ...
  return (
    <div className="min-h-full bg-background flex flex-col">
      ...
      <div className="px-4 mt-6 pb-6 space-y-3">
        ...
        <button
          onClick={skipPlayer}
          className="w-full py-3 rounded-2xl border border-border text-text-muted text-sm font-semibold active:scale-95 transition-transform"
        >
          Desconheço este jogador
        </button>
      </div>
    </div>
  )

...
// Gerencia o estado local de reações de um post + chamada pro backend.
// Aceita post ainda não carregado (null) — resincroniza quando ele chega.
export function useReactions(post, userId) {
  ...
  return { reactions, sortedReactions, handleReact }
}

// Pill de escolha de emoji + bolhas com a contagem de cada reação
export function ReactionBubbles({ sortedReactions, pickerOpen, setPickerOpen, onReact }) {
  return (
    <>
      {pickerOpen && (
        <div
...
// Link "X reações" que abre a lista de quem reagiu
export function ReactionsSummaryButton({ count, onClick }) {
...
// Bottom sheet com abas por emoji e a lista de quem reagiu com cada um
export function ReactionListModal({ reactions, onClose }) {
